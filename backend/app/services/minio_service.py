...
class MinIOService:
        ...
        for bucket in buckets:
            ...
            if not self.client.bucket_exists(bucket):
                try:
                    ...
                    logger.info("Bucket '%s' 创建成功", bucket)
                except S3Error as e:
                    # 如果创建失败，打印错误（可能只是已存在的并发错误）
                    logger.warning("创建 Bucket '%s' 时出错: %s", bucket, e)

            # 配置存储桶为公开读取
            self._set_bucket_public_read(bucket)
            logger.info("存储桶 %s 已配置为公开读取", bucket)
    ...
    def _set_bucket_public_read(self, bucket_name: str):
        ...
        try:
            ...
            logger.info("存储桶 %s 已设置为公开读取", bucket_name)
        except Exception as e:
            logger.warning("设置存储桶 %s 公开读取失败: %s", bucket_name, str(e))
