...
class Settings(BaseModel):
    ...
    # 开发环境允许 localhost、127.0.0.1 和常见局域网 IP 的前端开发服务跨域访问
    ...
    # 单批总上传大小（MB）
    max_batch_upload_size_mb: int = int(os.getenv("MAX_BATCH_UPLOAD_SIZE_MB", "200"))

    # YOLO 推理并发数。默认串行，降低模型线程安全和显存竞争风险。
    max_batch_workers: int = int(os.getenv("MAX_BATCH_WORKERS", "1"))
