...
# 配置日志：确保在所有子模块导入前配置，兼容 uvicorn 的日志系统
# NOTE: uvicorn 会先于应用代码配置日志，此时 basicConfig 会失效
# 这里采用防御性配置，确保应用模块的日志总能输出到控制台
root_logger = logging.getLogger()
root_logger.setLevel(logging.INFO)

# 如果根日志器没有处理器，添加一个 StreamHandler
...
app.add_middleware(
    ...
    allow_origin_regex=settings.cors_origin_regex,              # 开发环境允许本机和局域网地址
